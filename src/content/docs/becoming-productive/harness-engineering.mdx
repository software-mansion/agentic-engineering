...
information, tools, and constraints show up at the right time.
This page walks through several common ways to do that and the tradeoffs that
come with them.

## AGENTS.md

`AGENTS.md` is the lightest-weight way to steer an agent inside a
repository.
...
<ClapButton slug="becoming-productive/harness-engineering/agentsmd" />
...
A skill is just a well-named and well-described directory containing a `SKILL.md`
file with arbitrary Markdown content.
...
provide hard-to-reach up-to-date documentation about your toolchain or
...
There are many GitHub repositories that collect useful skills,
similar to the _Awesome X_ list repositories.
Many companies use these repositories as part of their marketing strategy,
providing skills that offer useful guidance for their products.
...
- <ExternalLink href="https://www.catonetworks.com/blog/cato-ctrl-weaponizing-claude-skills-with-medusalocker/" />.
  A seemingly harmless skill step in a GIF workflow was used to fetch and
  execute MedusaLocker ransomware, showing how a reviewed skill can still hide
  second-stage execution.
- <ExternalLink href="https://embracethered.com/blog/posts/2026/scary-agent-skills/" />.
  Hidden Unicode Tag characters were used to smuggle invisible instructions into
  a skill, which means visual source review alone may miss malicious behavior.

<ClapButton slug="becoming-productive/harness-engineering/skills" />
...
the filesystem, bash commands, and web browsing.
...
that connect your agent to remote services.

### Pros and cons

One advantage of MCP that CLIs and skills do not solve is authentication:
you can easily connect to MCP services using API keys or OAuth with
the UX provided by your harness.
...
Agents are trained a lot on Bash, and they are excellent on piping, awk, or jq.
...
The current consensus is to use MCPs mostly for connecting to external services,
like Linear, Figma, Slack or Sentry.
A good starting point for seeing which popular MCPs are available
is <ExternalLink href="https://github.com/mcp" />.
...
- <ExternalLink href="https://github.com/github/github-mcp-server">GitHub MCP</ExternalLink> provides
  ...
  and can do a lot by leveraging its JSON output and piping
...
- <ExternalLink href="https://context7.com/">Context7</ExternalLink> can be
  ...
  to <ExternalLink href="https://skills.sh/vercel-labs/agent-skills/vercel-react-best-practices">vercel-react-best-practices</ExternalLink>,
  or just more and more documentation websites support `Content: text/markdown`
  responses.
...
So... in such cases just make sure your agents will not perform destructive
actions on your behalf 🙂.

<ClapButton slug="becoming-productive/harness-engineering/mcp" />

## Subagents

![Subagents visualized](../../../assets/subagent.svg)

Subagents are extra agent runs that the main agent starts to handle a narrower
piece of work.
...
Exploration notes, false starts, and verbose tool output all compete with the
actual task for attention.
Offloading a bounded investigation to a subagent keeps the main thread smaller
and gives you a cleaner final result.
...
<ClapButton slug="becoming-productive/harness-engineering/subagents" />

## Hooks

Hooks are the part of the harness that runs deterministic logic around the
agent.
Different tools expose them differently,
but the idea is the same:
when something should happen every time,
do not rely on the model to remember it.

Hooks are a better home for repetitive enforcement than `AGENTS.md`.
Formatting code,
running linters,
asking for approval before sensitive commands,
posting notifications,
or opening a pull request are all examples of work that can often be attached
to a well-placed hook.
...
If a check fails, then the failure should come back with enough detail to guide
the next step.
...
<ClapButton slug="becoming-productive/harness-engineering/hooks" />
...
<ClapButton slug="becoming-productive/harness-engineering/when-to-use-what" />
